import { useState } from 'react'
import { Phone, X } from 'lucide-react'
import Navbar from './Navbar.jsx'
import Footer from './Footer.jsx'
import { BRAND } from '../data/site.js'

function WhatsAppIcon({ className = 'h-7 w-7' }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M.057 24l1.687-6.163a11.867 11.867 0 0 1-1.587-5.945C.16 5.335 5.495 0 12.05 0a11.817 11.817 0 0 1 8.413 3.488 11.824 11.824 0 0 1 3.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 0 1-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.82 9.82 0 0 0 1.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413z" />
    </svg>
  )
}

export default function Layout({ children }) {
  const [openFab, setOpenFab] = useState(false)
  const waNumber = BRAND.phonesRaw[0].replace('+', '')

  return (
    <div className="flex min-h-screen flex-col">
      <Navbar />
      <main className="flex-1">{children}</main>
      <Footer />

      {/* floating contact */}
      <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
        {openFab && (
          <div className="flex flex-col items-end gap-2">
            {BRAND.phones.map((p, i) => (
              <a
                key={p}
                href={`tel:${BRAND.phonesRaw[i]}`}
                className="flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-navy shadow-premium ring-1 ring-slate-100"
              >
                <Phone className="h-4 w-4 text-slateblue" /> {p}
              </a>
            ))}
          </div>
        )}

        <button
          onClick={() => setOpenFab((v) => !v)}
          className="flex h-14 w-14 items-center justify-center rounded-full bg-crimson text-white shadow-premium transition hover:scale-105"
          aria-label="Call us"
        >
          {openFab ? <X className="h-6 w-6" /> : <Phone className="h-6 w-6" />}
        </button>

        {/* sticky WhatsApp */}
        <a
          href={`https://wa.me/${waNumber}?text=${encodeURIComponent(
            'Hello Vidya Education Group, I would like to know more about your services.'
          )}`}
          target="_blank"
          rel="noreferrer"
          title={`WhatsApp ${BRAND.phones[0]}`}
          aria-label={`Chat on WhatsApp ${BRAND.phones[0]}`}
          className="group relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-premium transition hover:scale-105"
        >
          <span className="absolute inset-0 animate-ping rounded-full bg-[#25D366] opacity-30" />
          <WhatsAppIcon className="relative h-7 w-7" />
          <span className="pointer-events-none absolute right-16 hidden whitespace-nowrap rounded-lg bg-navy px-3 py-1.5 text-xs font-semibold text-white shadow-premium group-hover:block">
            {BRAND.phones[0]}
          </span>
        </a>
      </div>
    </div>
  )
}
